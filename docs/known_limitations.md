...
under the old recursive approach on the newer API, and rather than risk
producing subtly wrong chain physics, the extension just declines to run the
recursive path at all on 11.3+, with a one-time console warning. Geant4
11.2.2 remains the canonical, fully-supported version for this reason.
